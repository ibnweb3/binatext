...
- **Budget ~$0:** Workers free plan, Groq free tier (model), Workers AI free
  tier (fallback), SMS Gateway for Android / sms-gate.app free tier (SMS —
  runs on any Android + any SIM, incl. a Nigerian line). Anthropic Haiku (~$1 total) is
...
- `src/sms/*` — `SmsProvider` interface + `smsgate` (primary) / `twilio` (fallback) adapters
...
- `npm run spike:smsgate -- serve|send` — capture sms-gate.app's real inbound shape / test outbound
...
with a funded Agentic sub-account, an Android phone running sms-gate.app, and — for
